...
- v4.3.98
  - **动态调度诊断 v2.2**：影子评分增加候选特征拆解、预计下载时间和托管下载器效率
  - 新增下载器 5/15/60 分钟托管效率样本与任务结果回填
  - 影子决策和任务传输事件按决策 ID 关联
  - 仍不执行动态调度，不影响现有业务行为

- v4.3.97
  - **动态调度诊断**：新增下载器配置、全局与托管资源、任务 swarm、影子调度决策和传输事件记录
  - 第一阶段只记录推荐下载器，不改变现有大小分配和任务调度行为

- v4.3.96
  - **诊断记录 v2**：独立 v2 诊断库，候选去重并新增刷流汇总
  - 添加成功的候选会关联任务 hash 与来源 leechers/seeders
  - 修复真实完成判断与下载器下载中任务数
  - 记录上传峰值、最后上传、最终进度等上传窗口字段

- v4.3.95
  - **诊断导出可用**：插件数据页导出链接自动携带 API Token，点击即可下载最近 1/3/7/30 天数据

- v4.3.94
  - **诊断查看入口**：插件数据页新增“独立诊断记录”概览与最近 1/3/7/30 天导出链接
...
- v4.3.91
  - **多下载器大小分配**：新增“下载器分配”标签页，可启用多个下载器并按种子大小自动分流
  - 每个下载器可独立设置最大同时下载数、总上传/下载带宽、保种体积和单任务上传/下载限速
  - 默认关闭，关闭时保持原有单下载器流程；开启后任务记录会标记所属下载器，检查、上传保护、删种和统计均按下载器分组执行

- v4.3.72
  - **免费到期缓存**：免费种子记录到期时间，检查时优先按缓存判断临期删除，仅在进入兜底复查窗口时请求详情页，降低站点详情页压力
  - **删除范围**：已完成或已做种任务不参与免费到期检测，继续交给常规做种删种规则处理

- v4.3.71
  - **上传保护**：修复恢复限速语义，连续达标首次 `restore_limit` 恢复到上传保护基础下载限速，后续 `release_limit` 才完全放开下载限速

- v4.3.70
  - **上传保护**：实际传输观察期结束后，检查间采样未就绪时可使用下载器当前速度兜底计数；精简日志保留计数未开始的诊断提示，配置保存不重置上传保护运行计数

- v4.3.69
  - **精简日志**：配置写回和初始化时保留一条关键确认日志，显示日志模式、刷流间隔、检查间隔和开启时间段状态，完整配置快照仍降为调试日志

- v4.3.68
  - **秒级间隔**：新增刷流间隔和检查间隔秒级配置，刷流与检查服务均按秒级 interval 调度
  - **开启时间段**：新增“启用开启时间段”开关；未启用或时间段为空时默认全天运行，填写有效时间段后才限制刷流时段
  - **日志与配置**：修复日志模式写回遗漏导致重启后恢复完整日志的问题，精简模式下服务启动等普通日志降为调试日志
  - **上传保护**：已处于完全放开状态且无待处理动作时，不再重复向 qB 下发 `release_limit`
  - **失去免费检测**：详情页免费状态连续无法判断时进入退避重试，并记录安全诊断信息，避免每轮刷屏和频繁请求

- v4.3.67
  - **精简日志**：继续压低站点抓取普通信息，上传保护限速、恢复、放开日志增加种子名称，便于确认操作对象

- v4.3.66
  - **精简日志**：精简模式下压低站点抓取链路的普通日志，并合并上传保护成功动作日志，减少重复输出

- v4.3.65
  - **日志模式**：保留完整、摘要、精简三种模式；精简模式隐藏常规轮询流程，仅保留新增种子、删种命中、上传保护动作、错误和警告

- v4.3.57
  - **热修复**：重建删除规则标签页表单结构，修复因代码清理导致的配置项全部消失的严重 BUG

- v4.3.64
  - **日志模式**：修复日志模式设置项不可见的问题，将日志模式移至更多配置页并保留默认完整日志
...
- **刷流服务**：按 `brush_interval_seconds` 设置的秒级间隔运行，用于请求站点下载刷流种子，默认 600 秒。
- **刷流检查服务**：按 `check_interval_seconds` 设置的秒级间隔运行，用于同步检查下载器的刷流种子信息、删除种子和更新统计，默认 150 秒。

`brush_interval_minutes` 为旧版本兼容字段。未设置 `brush_interval_seconds` 时，会按旧分钟字段自动换算为秒。`cron` 字段保留兼容写回，不再改写分钟步进；需要限制刷流时段时，请使用“启用开启时间段”和“开启时间段”。
...
| 刷流间隔（秒）         | `brush_interval_seconds` | 控制刷流服务新增种子的调度间隔       | 默认 600，建议 180 到 600；未配置时兼容旧 `brush_interval_minutes`                                                 |
| 检查间隔（秒）         | `check_interval_seconds` | 控制刷流检查服务的调度间隔           | 默认 150，建议 150 到 300                                                                                         |
...
| 日志模式               | `log_mode`           | 控制日志可见性                       | `full` 完整日志；`summary` 压低普通判定日志；`concise` 精简日志，仅保留新增种子、删种命中、上传保护动作、错误和警告 |
| 启用开启时间段         | `active_time_range_enabled` | 是否启用刷流时间段限制               | 关闭时全天运行                                                                                                    |
| 开启时间段             | `active_time_range`  | 设置插件刷流的活动时间段             | 示例：00:00-08:00；未启用或留空时默认全天运行                                                                      |
| 运行时段/周期范围      | `cron`               | 旧版本兼容字段                       | 保留写回兼容，不再决定秒级调度频率                                                                                |
| 新增种子间隔（分钟）   | `brush_interval_minutes` | 旧版本兼容字段                       | 未配置 `brush_interval_seconds` 时用于换算秒级刷流间隔                                                            |
...
多下载器大小分配默认关闭，不影响原有单下载器配置。需要启用时，在配置页打开“下载器分配”标签页，开启“启用多下载器大小分配”后，MoviePilot 中已配置的下载器会以可视化分组显示。

每个下载器分组内可设置：

- 是否参与大小分配，以及接收的种子最小/最大大小（GB）
- 最大同时下载数、总上传带宽上限、总下载带宽上限、保种体积
...
- `delete_unknown_free_after_minutes`：未知免费时间兜底删除分钟，默认 `0` 关闭；未缓存到免费到期时间时只按本地计时删除，不在检查阶段访问详情页
- `seed_ratio`：分享率
