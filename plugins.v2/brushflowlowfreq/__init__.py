...
class DiagnosticRecorder:
    ...
    def _safe_execute(self, sql, params=()):
        try:
            ...
            logger.warning("brushflowlowfreq 诊断数据库写入失败", exc_info=True)
            return None
    ...
    def commit(self):
        try:
            ...
            logger.warning("brushflowlowfreq 诊断数据库提交失败", exc_info=True)

    def record_brush_start(self, site, started_at=None):
        cursor = self._safe_execute(
            "INSERT INTO brush_runs(started_at, site) VALUES(?, ?)",
            (float(started_at or time.time()), str(site or "")),
        )
    ...
    def record_candidate(self, run_id, site, torrent, decision, reject_reason=None,
        ...
        if not torrent_key:
            return
        title = str(getattr(torrent, "title", "") or torrent_key)
        site = str(site or "")
        seen_at = time.time()
        size_bytes = self._number(getattr(torrent, "size", None))
        seeders = self._number(getattr(torrent, "seeders", None))
        leechers = self._number(getattr(torrent, "leechers", None))
        if leechers is None:
            leechers = self._number(getattr(torrent, "peers", None))
        is_free = None
        free_remaining_minutes = self._number(getattr(torrent, "free_remaining_minutes", None))
        try:
            download_factor = getattr(torrent, "downloadvolumefactor", None)
            upload_factor = getattr(torrent, "uploadvolumefactor", None)
            if download_factor is not None:
                is_free = 1 if float(download_factor) == 0 else 0
            elif upload_factor is not None:
    ...
    def record_task_added(self, task_hash, torrent_task=None):
        ...
        self._safe_execute(
            ...
            INSERT OR IGNORE INTO task_profiles(
                task_hash, site, title, size_bytes, downloader, added_at
            ) VALUES(?, ?, ?, ?, ?, ?)
            """,
            (
                task_hash,
                str(torrent_task.get("site_name") or ""),
                str(torrent_task.get("title") or task_hash),
                self._number(torrent_task.get("total_size") or torrent_task.get("size")),
                str(torrent_task.get("downloader") or ""),
                self._number(torrent_task.get("time")) or time.time(),
            ),
        )
        self.record_task_event(task_hash, time.time(), "added", {"source": "brush"})

    def record_task_sample(self, task_hash, sampled_at, downloader, torrent_info=None,
                           torrent_task=None):
        task_hash = str(task_hash or "")
        if not task_hash:
            return
        ...
        self._safe_execute(
            """
            INSERT INTO task_samples(
                task_hash, sampled_at, downloader, state, downloaded, uploaded,
                progress, dl_speed, up_speed, interval_downloaded, interval_uploaded,
                interval_downspeed, interval_upspeed, seeding_time, completion_on,
                upload_protection_stage, upload_protection_low_streak,
                upload_protection_no_upload_streak, upload_protection_good_streak
            ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                task_hash,
                float(sampled_at or time.time()),
                str(downloader or torrent_task.get("downloader") or ""),
                str(torrent_info.get("state") or torrent_task.get("state") or ""),
                self._number(torrent_info.get("downloaded") if torrent_info else torrent_task.get("downloaded")),
                self._number(torrent_info.get("uploaded") if torrent_info else torrent_task.get("uploaded")),
                self._number(torrent_info.get("progress") if torrent_info else torrent_task.get("progress")),
                self._number(torrent_info.get("dlspeed") if torrent_info else torrent_task.get("dlspeed")),
                self._number(torrent_info.get("upspeed") if torrent_info else torrent_task.get("upspeed")),
                self._number(torrent_task.get("last_check_interval_downloaded")),
                self._number(torrent_task.get("last_check_interval_uploaded")),
                self._number(torrent_task.get("last_check_interval_downspeed")),
                self._number(torrent_task.get("last_check_interval_upspeed")),
                self._number(torrent_info.get("seeding_time") if torrent_info else torrent_task.get("seeding_time")),
                self._number(torrent_info.get("completion_on") if torrent_info else torrent_task.get("completion_on")),
                str(torrent_task.get("upload_protection_stage") or ""),
                self._number(torrent_task.get("upload_protection_low_streak")),
                self._number(torrent_task.get("upload_protection_no_upload_streak")),
                self._number(torrent_task.get("upload_protection_good_streak")),
            ),
        )
    ...
    def record_task_event(self, task_hash, event_at, event_type, detail=None):
        ...
        try:
            ...
            detail_json = None
        self._safe_execute(
            "INSERT INTO task_events(task_hash, event_at, event_type, detail) VALUES(?, ?, ?, ?)",
    ...
    def record_downloader_sample(self, sampled_at, downloader, snapshot=None):
        ...
        self._safe_execute(
            """
            INSERT INTO downloader_samples(
                sampled_at, downloader, active_count, downloading_count,
                uploading_count, upload_speed, download_speed, error
            ) VALUES(?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                float(sampled_at or time.time()),
                str(downloader or ""),
                self._number(snapshot.get("active_count")),
                self._number(snapshot.get("downloading_count")),
                self._number(snapshot.get("uploading_count")),
                self._number(snapshot.get("upload_speed")),
                self._number(snapshot.get("download_speed")),
    ...
    def finalize_task(self, task_hash, torrent_task=None, deleted_type=None, deleted_reason=None,
        ...
        self._safe_execute(
            ...
            SET first_downloaded_at=COALESCE(first_downloaded_at, ?),
                ...
                deleted_type=COALESCE(deleted_type, ?),
            ...
            (
                ...
                self._number(torrent_task.get("download_dashboard_completed_time")),
                deleted_at,
                str(deleted_type or torrent_task.get("deleted_type") or "") or None,
                str(deleted_reason or torrent_task.get("deleted_reason") or "") or None,
                self._number(torrent_task.get("uploaded")),
    ...
    def _ensure_task_profile(self, task_hash, torrent_task=None):
        torrent_task = torrent_task or {}
        self._safe_execute(
            """
            INSERT OR IGNORE INTO task_profiles(
    ...
    def query_task_samples(self, task_hash, limit=10000):
        try:
            rows = self._conn.execute(
                "SELECT * FROM task_samples WHERE task_hash=? ORDER BY sampled_at DESC LIMIT ?",
                (task_hash, int(limit or 10000)),
            ).fetchall()
            return [dict(row) for row in rows]
        except Exception:
            return []

    def query_events(self, task_hash, limit=10000):
        try:
            rows = self._conn.execute(
                "SELECT * FROM task_events WHERE task_hash=? ORDER BY event_at DESC LIMIT ?",
                (task_hash, int(limit or 10000)),
            ).fetchall()
            return [dict(row) for row in rows]
        except Exception:
    ...
    def query_downloader_samples(self, downloader, limit=10000):
        try:
            rows = self._conn.execute(
                "SELECT * FROM downloader_samples WHERE downloader=? ORDER BY sampled_at DESC LIMIT ?",
                (downloader, int(limit or 10000)),
            ).fetchall()
            return [dict(row) for row in rows]
        except Exception:
            return []

    def count_samples(self, task_hash):
        try:
            row = self._conn.execute(
                "SELECT COUNT(*) AS c FROM task_samples WHERE task_hash=?",
                (task_hash,),
            ).fetchone()
...
class BrushConfig:
    ...
    def __init__(self, config: dict, process_site_config=True):
        ...
        self.delete_unknown_free_after_minutes = self.__parse_number(
            config.get("delete_unknown_free_after_minutes", 0)
        )
        ...
        try:
            retention_days = int(float(config.get("diagnostic_retention_days", 30) or 30))
        except (TypeError, ValueError):
            retention_days = 30
        self.diagnostic_retention_days = max(1, retention_days)
    ...
    def __initialize_site_config(self):
        ...
        allowed_fields = {
            ...
            "delete_unknown_free_after_minutes",
            "qb_category",
            "site_hr_active",
            "site_skip_tips",
            "include_second_page",
...
class BrushFlowLowFreq(_PluginBase):
    ...
    def __is_external_fetch_noise_log_message(message: str) -> bool:
        ...
        return (
                "开始请求：" in message
                or ("搜索完成" in message and "返回数据：" in message)
                or (message.startswith("开始获取站点 ") and " 最新种子" in message)
        )
    ...
    def __with_quiet_external_fetch_logs(self, func, *args, **kwargs):
        ...
        original_debug = getattr(logger, "debug", None)
        app_logger_info_wrapped = False

        def quiet_info(*log_args, **log_kwargs):
            message = " ".join(str(arg) for arg in log_args)
            if self.__is_external_fetch_noise_log_message(message):
                if callable(original_debug):
                    return original_debug(*log_args, **log_kwargs)
                return None
            return original_info(*log_args, **log_kwargs)
        ...
        try:
            ...
            if callable(original_info):
                try:
                    setattr(logger, "info", quiet_info)
                    app_logger_info_wrapped = True
                except Exception:
                    app_logger_info_wrapped = False
            return func(*args, **kwargs)
        finally:
            if app_logger_info_wrapped:
                try:
                    setattr(logger, "info", original_info)
                except Exception:
                    pass
    ...
    @staticmethod
    def __upload_protection_torrent_title(torrent_task: Optional[dict],
                                          torrent_info: Optional[dict] = None,
                                          torrent_hash: str = "") -> str:
        candidates = []
        if isinstance(torrent_task, dict):
            candidates.extend([
                torrent_task.get("title"),
                torrent_task.get("name"),
            ])
    ...
    def __format_upload_protection_torrent_part(self, torrent_task: Optional[dict],
                                                torrent_info: Optional[dict] = None,
                                                torrent_hash: str = "") -> str:
        title = self.__upload_protection_torrent_title(
            torrent_task=torrent_task,
            torrent_info=torrent_info,
            torrent_hash=torrent_hash
        )
        return f"，种子：{title}" if title else ""
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        """
        注册插件自定义API端点，用于外部程序获取刷流数据
        """
        return [
            {
                "path": "/summary",
                "endpoint": self.__api_summary,
                "methods": ["GET"],
            },
            {
                "path": "/daily_compare",
                "endpoint": self.__api_daily_compare,
                "methods": ["GET"],
            },
            {
                "path": "/tasks",
                "endpoint": self.__api_tasks,
                "methods": ["GET"],
            },
            {
                "path": "/trend",
                "endpoint": self.__api_trend,
                "methods": ["GET"],
            },
            {
                "path": "/qb_tasks",
    ...
    def __api_summary(self):
        ...
        return {
            "today": {
                "uploaded": today_record.get("uploaded", 0) if isinstance(today_record, dict) else 0,
                "downloaded": today_record.get("downloaded", 0) if isinstance(today_record, dict) else 0,
            },
            "month": {
                "uploaded": month_record.get("uploaded", 0) if isinstance(month_record, dict) else 0,
                "downloaded": month_record.get("downloaded", 0) if isinstance(month_record, dict) else 0,
            },
        }

    def __api_daily_compare(self):
        """返回本日和昨日上传/下载量对比（字节）"""
        try:
            now = datetime.now(tz=pytz.timezone(settings.TZ))
        except Exception:
        ...
        def _extract(record):
            if not isinstance(record, dict):
                return 0, 0
            return record.get("uploaded", 0), record.get("downloaded", 0)

        today_up, today_down = _extract(today_record)
        yesterday_up, yesterday_down = _extract(yesterday_record)

        return {
            "today": {"uploaded": today_up, "downloaded": today_down},
            "yesterday": {"uploaded": yesterday_up, "downloaded": yesterday_down},
        }

    def __api_tasks(self, limit: int = 5, offset: int = 0):
        """返回正在上传的刷流任务列表，按上传速度降序"""
        torrent_tasks = self.get_data("torrents") or {}
        ...
        now_ts = time.time()
        tasks = []
        for _hash, task in torrent_tasks.items():
            if not isinstance(task, dict):
                continue
            if task.get("deleted", False):
                continue

            # 最近一次检查间的上传速度（Byte/s），用于判断是否正在上传
            interval_upspeed = self.__number_or_none(task.get("last_check_interval_upspeed"))
            avg_upspeed = self.__number_or_none(task.get("avg_upspeed"))

            # 当前上传速度：优先使用检查间速度，否则用平均速度
            current_upspeed = (interval_upspeed if interval_upspeed is not None and interval_upspeed > 0
                               else avg_upspeed if avg_upspeed is not None else 0)

            # 只显示正在上传的任务（速度 > 0）
            if current_upspeed <= 0:
                continue
            ...
            size = int(self.__number_or_none(task.get("size")) or
                        self.__number_or_none(task.get("total_size")) or 0)
            uploaded = int(self.__number_or_none(task.get("uploaded")) or 0)
            downloaded = int(self.__number_or_none(task.get("downloaded")) or 0)

            progress = round(downloaded / size, 2) if size > 0 else 0.0
            ratio = round(float(self.__number_or_none(task.get("ratio")) or 0), 2)
        ...
        # 按上传速度降序
        tasks.sort(key=lambda t: t["avg_upspeed"], reverse=True)
        total = len(tasks)

        # 分页
        paged = tasks[offset:offset + limit]

        return {"tasks": paged, "total": total}

    def __api_qb_tasks(self, limit: int = 5, offset: int = 0):
        """直接从 qBittorrent 获取实时任务，分为正在下载中和正在上传中，仅包含有传输速度的任务"""
        downloader = self.downloader
        if not downloader:
            return {"downloading": [], "seeding": [], "dl_total": 0, "seed_total": 0}
        try:
            torrents, error = downloader.get_torrents()
            if error:
                return {"downloading": [], "seeding": [], "dl_total": 0, "seed_total": 0, "error": str(error)}
        except Exception as e:
            return {"downloading": [], "seeding": [], "dl_total": 0, "seed_total": 0, "error": str(e)}

        downloading_all = []
        seeding_all = []
        dl_states = {"downloading", "stalledDL", "forcedDL", "metaDL",
                      "queuedDL", "checkingDL", "allocating", "moving"}
        up_states = {"uploading", "stalledUP", "forcedUP", "queuedUP",
                      "checkingUP"}
        ...
        for torrent in (torrents or []):
            ...
            total_size = int(self.__number_or_none(info.get("total_size")) or 0)
            downloaded = int(self.__number_or_none(info.get("downloaded")) or 0)
            progress = round(downloaded / total_size, 2) if total_size > 0 else 0.0
            dlspeed = int(self.__number_or_none(info.get("dlspeed")) or 0)
            upspeed = int(self.__number_or_none(info.get("upspeed")) or 0)
            ...
            item = {
                ...
                "uploaded": int(self.__number_or_none(info.get("uploaded")) or 0),
                "ratio": round(float(self.__number_or_none(info.get("ratio")) or 0), 2),
            }

            if state in dl_states and dlspeed > 0:
                downloading_all.append(item)
            elif state in up_states and upspeed > 0:
                seeding_all.append(item)

        # 下载中：按下载速度降序
        downloading_all.sort(key=lambda t: t["dlspeed"], reverse=True)
        # 上传中：按上传速度降序
        seeding_all.sort(key=lambda t: t["upspeed"], reverse=True)

        dl_total = len(downloading_all)
        seed_total = len(seeding_all)

        dls = downloading_all[offset:offset + limit]
        seeds = seeding_all[offset:offset + limit]
    ...
    def __api_trend(self):
        ...
        hourly_statistic = self.get_data("hourly_statistic") or {}
        if not isinstance(hourly_statistic, dict):
            return {"points": []}
        ...
        for hour_offset in range(24, -1, -1):
            ...
            record = hourly_statistic.get(hour_key)
            if isinstance(record, dict):
                points.append({
                    "time": hour_dt.strftime("%Y-%m-%dT%H:00:00"),
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
                                                ...
                                                'content': [
                                                    {
                                                        ...
                                                        'props': {
                                                            ...
                                                            'placeholder': '默认1，保留兼容；检查阶段不再请求详情页'
                                                        }
                                                    }
                                                ]
                                            },
                                                ...
                                                'content': [
                                                    {
                                                        'component': 'VTextField',
                                                        'props': {
                                                            'model': 'delete_unknown_free_after_minutes',
                                                            'label': '未知免费时间兜底删除（分钟）',
                                                            'placeholder': '默认0关闭，未缓存到期时间时本地等待后删除'
                                                        }
                                                    }
                                                ]
                                            },
        ...
        defaults = {
            ...
            "delete_unknown_free_after_minutes": 0,
    ...
    def stop_service(self):
        ...
        退出插件
        """
        self.__stop_diagnostic_recorder()
        try:
            if self._scheduler:
    ...
    def __diagnostic_db_path(self):
        try:
            log_path = getattr(settings, "LOG_PATH", None)
            if log_path:
                base = Path(str(log_path))
            else:
                config_path = Path(str(getattr(settings, "CONFIG_PATH", "/config")))
                base = config_path.parent if config_path.suffix.lower() in {".yaml", ".yml"} else config_path
                base = base / "logs"
    ...
    def __start_diagnostic_recorder(self):
        self.__stop_diagnostic_recorder()
        brush_config = self._brush_config
        if not brush_config or not brush_config.diagnostic_enabled:
            return
        try:
            ...
            logger.warning("brushflowlowfreq 独立诊断记录启动失败", exc_info=True)

    def __stop_diagnostic_recorder(self):
        try:
    ...
    def check(self, downloader_name: str = None, group_hashes: Optional[Set[str]] = None,
        ...
        with lock:
            ...
            # 更新每小时上传/下载增量统计（供外部API使用）
            self.__update_hourly_statistics(torrent_tasks=torrent_tasks)
    ...
    def __apply_upload_protection_actions(self, torrents: List[Any], torrent_tasks: Dict[str, dict],
        ...
        for torrent in torrents:
            ...
            if action in {"limit", "strict_limit", "restore_limit", "release_limit"}:
                torrent_title = self.__upload_protection_torrent_title(
                    torrent_task=torrent_task,
                    torrent_info=torrent_info,
                    torrent_hash=torrent_hash
                )
    ...
    @staticmethod
    def __get_torrent_field(torrent: Any, field_name: str, default: Any = None) -> Any:
        if isinstance(torrent, dict):
            return torrent.get(field_name, default)
        return getattr(torrent, field_name, default)

    @staticmethod
    def __normalize_free_cache_match_key(value: Any) -> str:
        if value is None:
            return ""
        return re.sub(r"\s+", " ", str(value)).strip().lower()

    def __cache_free_expire_from_brush_list(self, siteinfo: Any, torrents: List[Any],
                                           torrent_tasks: Dict[str, dict]) -> int:
        if not torrents or not torrent_tasks:
            return 0

        torrents_by_page_url = {}
        torrents_by_title = {}
        for torrent in torrents:
            page_url = self.__normalize_free_cache_match_key(
                self.__get_torrent_field(torrent, "page_url")
            )
            title = self.__normalize_free_cache_match_key(
                self.__get_torrent_field(torrent, "title")
            )
            if page_url:
                torrents_by_page_url[page_url] = torrent
            if title:
                torrents_by_title[title] = torrent

        site_id = self.__get_torrent_field(siteinfo, "id")
        site_name = self.__get_torrent_field(siteinfo, "name", "")
        repaired_count = 0
        for torrent_task in torrent_tasks.values():
            if not isinstance(torrent_task, dict) or torrent_task.get("deleted"):
                continue
            if site_id is not None and str(torrent_task.get("site")) != str(site_id):
                continue
            if site_name and torrent_task.get("site_name") != site_name:
                continue
            if self.__get_cached_free_remaining_minutes(torrent_task=torrent_task) is not None:
                continue

            page_url = self.__normalize_free_cache_match_key(torrent_task.get("page_url"))
            title = self.__normalize_free_cache_match_key(torrent_task.get("title"))
            torrent = torrents_by_page_url.get(page_url) if page_url else None
            if torrent is None and title:
                torrent = torrents_by_title.get(title)
            if torrent is None:
                continue

            for field_name in ("freedate", "freedate_diff", "downloadvolumefactor", "description"):
                value = self.__get_torrent_field(torrent, field_name)
                if value not in (None, ""):
                    torrent_task[field_name] = value
            remaining = self.__cache_free_expire_from_task_fields(
                torrent_task=torrent_task,
                source="list"
            )
            if remaining is None:
    ...
    @staticmethod
    def __clear_free_unknown_state(torrent_task: dict) -> None:
        if not isinstance(torrent_task, dict):
            return
        torrent_task.pop("free_unknown_started_at", None)
        torrent_task.pop("free_unknown_last_reason", None)

    @staticmethod
    def __get_delete_unknown_free_after_minutes(brush_config: BrushConfig) -> float:
        try:
            value = float(brush_config.delete_unknown_free_after_minutes)
        except (TypeError, ValueError):
            return 0.0
        return value if value > 0 else 0.0

    def __evaluate_unknown_free_expire_fallback(self, site_name: str, torrent_task: dict,
                                                brush_config: BrushConfig,
                                                now_ts: float) -> Tuple[bool, str]:
        fallback_minutes = self.__get_delete_unknown_free_after_minutes(brush_config=brush_config)
        self.__clear_free_undetermined_state(torrent_task)
        if fallback_minutes <= 0:
            self.__clear_free_unknown_state(torrent_task)
            self.__log_summary_routine(
                f"失去免费删种无缓存跳过：站点={site_name}，标题={torrent_task.get('title', '')}，"
                "未获取到免费到期时间，检查阶段不访问站点"
            )
            return False, "未获取到免费到期时间，检查阶段不访问站点"

        started_at = self.__number_or_none(torrent_task.get("free_unknown_started_at"))
        if started_at is None:
            started_at = float(now_ts)
            torrent_task["free_unknown_started_at"] = started_at
        torrent_task["free_unknown_last_reason"] = "未获取到免费到期时间"

        elapsed_minutes = max(0.0, (float(now_ts) - started_at) / 60)
        if elapsed_minutes >= fallback_minutes:
            self.__clear_free_unknown_state(torrent_task)
            return True, (
                f"未获取到免费到期时间，已等待 {elapsed_minutes:.0f} 分钟，"
                f"达到未知免费时间兜底 {fallback_minutes:.0f} 分钟，按配置执行彻底删除"
            )

        self.__log_summary_routine(
            f"失去免费删种未知免费时间兜底等待：站点={site_name}，标题={torrent_task.get('title', '')}，"
            f"已等待={elapsed_minutes:.0f} 分钟，兜底={fallback_minutes:.0f} 分钟"
        )
        return False, (
            f"未获取到免费到期时间，未知免费时间兜底等待 "
            f"{elapsed_minutes:.0f}/{fallback_minutes:.0f} 分钟，检查阶段不访问站点"
        )
    ...
    def __evaluate_no_free_condition_for_delete(self, site_name: str, torrent_task: dict) -> Tuple[bool, str]:
        ...
        return self.__evaluate_unknown_free_expire_fallback(
            site_name=site_name,
            torrent_task=torrent_task,
            brush_config=brush_config,
            now_ts=now_ts
    ...
    def __update_hourly_statistics(self, torrent_tasks: Dict[str, dict],
                                    now: Optional[datetime] = None) -> None:
        """
        根据插件托管任务的累计上传/下载量，更新每小时增量统计（供24h趋势图使用）。
        与 __update_daily_transfer_statistics 独立运行，使用独立的每小时基线标记。
        """
        if not isinstance(torrent_tasks, dict):
            return
        ...
        if now and isinstance(now, datetime):
            stat_hour = now.strftime("%Y-%m-%d-%H")
        else:
            try:
                stat_hour = datetime.now(tz=pytz.timezone(settings.TZ)).strftime("%Y-%m-%d-%H")
            except Exception:
                stat_hour = datetime.now().strftime("%Y-%m-%d-%H")
        ...
        for _hash, task in torrent_tasks.items():
            ...
            last_uploaded = self.__number_or_none(task.get("hourly_last_uploaded"))
            last_downloaded = self.__number_or_none(task.get("hourly_last_downloaded"))
            if last_uploaded is None or last_downloaded is None:
                task["hourly_last_hour"] = stat_hour
                task["hourly_last_uploaded"] = int(uploaded)
                task["hourly_last_downloaded"] = int(downloaded)
                continue
            ...
            hour_record = hourly_statistic.get(stat_hour)
            ...
            hour_record["date"] = stat_hour
            hour_record["uploaded"] = int(
                (self.__number_or_none(hour_record.get("uploaded")) or 0) + upload_delta
            )
            hour_record["downloaded"] = int(
                (self.__number_or_none(hour_record.get("downloaded")) or 0) + download_delta
            )
            hour_record["updated_at"] = updated_at
            hourly_statistic[stat_hour] = hour_record

        # 清理超过48小时的数据
        cutoff_ts = updated_at - 48 * 3600
        stale_keys = [
            k for k, v in hourly_statistic.items()
            if isinstance(v, dict) and (v.get("updated_at") or 0) < cutoff_ts
        ]
        for k in stale_keys:
            del hourly_statistic[k]

        self.save_data("hourly_statistic", hourly_statistic)
        self.save_data("torrents", torrent_tasks)
    ...
    def __validate_and_fix_config(self, config: dict = None) -> bool:
        ...
        config_number_attr_to_desc = {
            ...
            "delete_unknown_free_after_minutes": "未知免费时间兜底删除分钟",
    ...
    def __update_config(self, brush_config: BrushConfig = None, reason: str = ""):
        ...
        config_mapping = {
            ...
            "delete_unknown_free_after_minutes": brush_config.delete_unknown_free_after_minutes,
            ...
            "diagnostic_enabled": brush_config.diagnostic_enabled,
            "diagnostic_retention_days": brush_config.diagnostic_retention_days,
    ...
    def __apply_qb_upload_protection_action(self, torrent_hash: str, action: str, brush_config: BrushConfig,
                                            torrent_task: dict, site_name: str = "", reason: str = "",
                                            torrent_title: str = "") -> bool:
        """
