...
class BrushFlowLowFreq(_PluginBase):
        ...
        return [
            {
                ...
                "methods": ["GET"],
                "summary": "获取刷流汇总 - 本日/本月上传下载量",
            },
            {
                "path": "/daily_compare",
                "endpoint": self.__api_daily_compare,
                "methods": ["GET"],
                "summary": "获取本日与昨日上传下载对比",
            },
            {
                "path": "/tasks",
    ...
    async def __api_summary(self, request):
        """返回本日和本月上传/下载量（字节）"""
        try:
    ...
    async def __api_daily_compare(self, request):
        """返回本日和昨日上传/下载量对比（字节）"""
        try:
            now = datetime.now(tz=pytz.timezone(settings.TZ))
        except Exception:
    ...
    async def __api_tasks(self, request):
        """返回活跃刷流任务列表（未标记删除的种子）"""
        ...
        for _hash, task in torrent_tasks.items():
            ...
            add_time = self.__number_or_none(task.get("time")) or now_ts
            elapsed = max(now_ts - add_time, 1)
            avg_upspeed = int(uploaded / elapsed)
        ...
        tasks.sort(key=lambda t: t["uploaded"], reverse=True)
        return {"tasks": tasks[:50]}

    async def __api_trend(self, request):
        """返回过去24小时每小时的流量趋势"""
        try:
            now = datetime.now(tz=pytz.timezone(settings.TZ))
        except Exception:
