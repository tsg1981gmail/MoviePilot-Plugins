...
class BrushFlowLowFreq(_PluginBase):
    ...
    def __api_qb_tasks(self):
        """直接从 qBittorrent 获取实时任务，分为正在下载中和已做种"""
        downloader = self.downloader
        if not downloader:
            return {"downloading": [], "seeding": [], "error": "下载器未连接"}

        try:
            torrents, error = downloader.get_torrents()
            if error:
                return {"downloading": [], "seeding": [], "error": str(error)}
        except Exception as e:
            return {"downloading": [], "seeding": [], "error": str(e)}
        ...
        for torrent in (torrents or []):
            ...
            if state in dl_states:
                downloading_list.append(item)
            elif state in up_states:
                seeding_list.append(item)

        # 下载中：按下载速度降序
        downloading_list.sort(key=lambda t: t["dlspeed"], reverse=True)
        # 已做种：按上传速度降序
        seeding_list.sort(key=lambda t: t["upspeed"], reverse=True)

        return {
            "downloading": downloading_list,
            "seeding": seeding_list,
        }
